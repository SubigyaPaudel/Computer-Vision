...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        ### YOUR CODE HERE
        wx = Ix[y-w: y+w+1, x-w: x+w+1]
        wy = Iy[y-w: y+w+1, x-w: x+w+1]
        wt = -It[y-w: y+w+1, x-w: x+w+1].reshape(-1,1)
        #extracting the window of size w from a particular keypoint from the arrays A1 and A2
        A = np.c_[wx.reshape(-1,1), wy.reshape(-1,1)]
        #constructing the matrix A
        d = np.dot(np.linalg.inv(A.T.dot(A)), A.T.dot(wt))
        #solving for the least square error for the flow vector
...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        G = np.array([[np.sum(wx**2), np.sum(wx*wy)], [np.sum(wx*wy), np.sum(wy**2)]])
        ...
        for k in range(num_iters):
            ...
            bk = np.array([np.sum(Ik*wx), np.sum(Ik*wy)])
            vk = G_inv.dot(bk)
            ### END YOUR CODE
...
def pyramid_lucas_kanade(img1, img2, keypoints,
    ...
    for L in range(level, -1, -1):
        ...
        J = pyramid2[L]
        p = keypoints / scale**L
        d = iterative_lucas_kanade(I, J, p, window_size=window_size, num_iters=num_iters, g=g)
        g = scale * (g + d)
        ### END YOUR CODE
...
def compute_error(patch1, patch2):
    ...
    error = 0
    ### YOUR CODE HERE
    normalized_patch1 = (patch1 - np.mean(patch1)) / np.std(patch1)
    normalized_patch2 = (patch2 - np.mean(patch2)) / np.std(patch2)
    error = np.mean(np.square(normalized_patch1- normalized_patch2))
    ### END YOUR CODE
    return error
